...
function smoothAngleValue(id, currentAngle) {
  ...
  smoothAngles[id] = alpha * currentAngle + (1 - alpha) * smoothAngles[id];
...
async function process_frame() {
  ...
  try {
    ...
    if (ctxColor)
      ctxColor.drawImage(video, 0, 0, canvasColor.width, canvasColor.height);
  ...
  detections.forEach((det) => {
    ...
    let colorBorder, colorArrow;
    if (det.id === 1) {
      colorBorder = "#7fff29"; // violett
      colorArrow = "#7fff29";
    } else if (det.id === 2) {
      colorBorder = "#9463e5"; // grün
      colorArrow = "#9463e5";
    } else {
...
function drawArrow(ctx, x, y, angleDeg, color = "#7fff29") {
